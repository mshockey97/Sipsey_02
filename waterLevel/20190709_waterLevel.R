...
data_dir<-"C:\\Users\\cnjones7\\Box Sync\\My Folders\\Research Projects\\SWI\\PT_Data\\20190709_Downloads\\"
...
offset<-df %>% 
  ...
  mutate(offset = replace_na(offset, 0))
...
h_record<-read_csv("C:\\Users\\cnjones7\\Box Sync\\My Folders\\Research Projects\\SWI\\PT_Data\\20190315_Downloads\\20190315_waterLevel.csv")
...
#Add offset to match historic
updated<-waterDepth %>%
  mutate(waterDepth = waterDepth + 0.046)
...
#Add offset to match historic
updated<-waterDepth %>%
  mutate(waterDepth = waterDepth +0.0183)
...
#Add offset to match historic
updated<-waterDepth %>%
  mutate(waterDepth = waterDepth - 0.1293)
...
#Add offset to match historic
updated<-waterDepth %>%
  mutate(waterDepth = waterDepth -0.1351)
...
#Add offset to match historic
updated<-waterDepth %>%
  mutate(waterDepth = waterDepth -0.0629)
...
#Add offset to match historic
updated<-waterDepth %>%
  mutate(waterDepth = waterDepth - 0.3428)
